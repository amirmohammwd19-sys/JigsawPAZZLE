...
import { getStats, saveStats, getRecord, saveRecord } from './storage';
...
function TutorialScreen({ onBack }: { onBack: () => void }) {
  ...
  const handleNext = () => {
    ...
    else { markTutorialAsSeen(); onBack(); }
  };
  const handleSkip = () => { markTutorialAsSeen(); onBack(); };
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 flex items-center justify-center p-4">
      <div className="max-w-2xl w-full bg-white/10 backdrop-blur-md rounded-3xl p-8 border border-white/20 shadow-2xl">
        ...
        <div className="flex gap-4">
          <button onClick={handleSkip} className="flex-1 px-6 py-3 bg-white/10 hover:bg-white/20 rounded-xl text-white font-bold">رد شدن</button>
          <button onClick={handleNext} className="flex-1 px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl text-white font-bold">{currentStep === TUTORIAL_STEPS.length - 1 ? 'شروع بازی' : 'بعدی'}</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {ACHIEVEMENTS.map(a => {
            ...
            return (
              <div key={a.id} className={`rounded-2xl p-6 border ${unlocked ? 'bg-gradient-to-br from-yellow-500/20 to-orange-500/20 border-yellow-400/30' : 'bg-white/5 border-white/10 opacity-60'}`}>
          ...
          <div className="flex flex-col md:flex-row gap-8 items-center">
            ...
            <div className="w-full md:w-1/2 text-center md:text-left">
              ...
              {completed ? (
                ...
                <button onClick={handleStart} className="w-full px-6 py-3 bg-gradient-to-r from-orange-500 to-red-500 rounded-xl text-white font-bold">🚀 شروع چالش</button>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {POWER_UPS.map(powerUp => {
            ...
            return (
              <div key={powerUp.id} className={`rounded-2xl p-6 border ${canAfford ? 'bg-gradient-to-br from-purple-500/20 to-pink-500/20 border-purple-400/30' : 'bg-white/5 border-white/10 opacity-50'}`}>
                <div className="flex items-start gap-4">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {THEMES.map(theme => {
            ...
            return (
              <div key={theme.id} onClick={() => isUnlocked && handleSelectTheme(theme.id)} className={`rounded-2xl p-6 border-2 ${isCurrent ? 'border-yellow-400 scale-105' : isUnlocked ? 'border-white/20 hover:border-white/40 cursor-pointer' : 'border-white/10 opacity-50 cursor-not-allowed'}`}>
                <div className={`bg-gradient-to-br ${theme.background} rounded-xl p-4 mb-3`}>
                  <div className="text-5xl text-center">{theme.emoji}</div>
                </div>
                <h3 className="text-white font-bold text-center">{theme.name}</h3>
                {isCurrent && <div className="text-center text-yellow-300 text-sm font-bold mt-2">✓ فعال</div>}
                {!isUnlocked && <div className="text-center text-purple-300 text-sm mt-2">🔒 سطح {theme.unlockLevel}</div>}
              </div>
            );
          })}
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
            <div className="bg-gradient-to-br from-red-800 to-pink-800 rounded-3xl p-8 max-w-md w-full text-center border border-white/20">
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowConfirmReset(false)} className="flex-1 px-6 py-3 bg-white/10 rounded-xl text-white font-bold">انصراف</button>
                <button onClick={handleResetAll} className="flex-1 px-6 py-3 bg-red-500 rounded-xl text-white font-bold">حذف کامل</button>
              </div>
            </div>
          </div>
        )}
