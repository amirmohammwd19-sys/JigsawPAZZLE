...
export function getLeaderboard(): LeaderboardEntry[] { try { const d = localStorage.getItem(LEADERBOARD_KEY); if (d) return JSON.parse(d); } catch {} return []; }
export function saveToLeaderboard(entry: LeaderboardEntry): void { try { const lb = getLeaderboard(); lb.push(entry); lb.sort((a, b) => b.score - a.score); localStorage.setItem(LEADERBOARD_KEY, JSON.stringify(lb.slice(0, 10))); } catch {} }

// Auto-save
export interface GameSave { puzzleName: string; pieces: any[]; moves: number; time: number; date: string; }
export function saveGame(save: GameSave): void { try { localStorage.setItem(AUTOSAVE_KEY, JSON.stringify(save)); } catch {} }
export function loadGame(): GameSave | null { try { const d = localStorage.getItem(AUTOSAVE_KEY); if (d) return JSON.parse(d); } catch {} return null; }
export function clearAutoSave(): void { try { localStorage.removeItem(AUTOSAVE_KEY); } catch {} }

// Export/Import
export function exportSaveData(): string { try { return JSON.stringify({ stats: localStorage.getItem(STATS_KEY), records: localStorage.getItem(RECORDS_KEY), leaderboard: localStorage.getItem(LEADERBOARD_KEY), exportDate: new Date().toISOString() }, null, 2); } catch { return ''; } }
export function importSaveData(json: string): boolean { try { const d = JSON.parse(json); if (d.stats) localStorage.setItem(STATS_KEY, d.stats); if (d.records) localStorage.setItem(RECORDS_KEY, d.records); if (d.leaderboard) localStorage.setItem(LEADERBOARD_KEY, d.leaderboard); return true; } catch { return false; } }
